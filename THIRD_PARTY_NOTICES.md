...
The source archive does not vendor Python packages, Pandoc, Chromium, or a Python runtime. Reproduction installs the exact packages named in `requirements-lock.txt` from their publishers. Principal direct dependencies and their upstream licenses are:
